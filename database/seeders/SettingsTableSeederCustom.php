...
class SettingsTableSeederCustom extends Seeder
{
    ...
    public function run()
    {
        ...
        $setting = $this->findSetting("multi-vendor.guest_checkout");
        if (!$setting->exists) {
            $setting->fill(["display_name" => "Guest Checkout", "value" => "1", "details" => json_encode([
                "validation" => [
                    "rule" => "in:0,1,on,off"
                ]
            ]), "type" => "checkbox", "order" => "51", "group" => "Multi-vendor", ])->save();
        }
        $setting = $this->findSetting("multi-vendor.is_otp_verify");
        if (!$setting->exists) {
            $setting->fill(["display_name" => "Guest Email OTP Verification", "value" => "1", "details" => json_encode([
                "validation" => [
                    "rule" => "in:0,1,on,off"
                ],
                "description" => "This will work only if above Guest Checkout option is enabled",
            ]), "type" => "checkbox", "order" => "52", "group" => "Multi-vendor", ])->save();
        }
    }
