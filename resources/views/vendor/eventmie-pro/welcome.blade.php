                    ...
                    @foreach ($cities_events as $key => $item)
                        ...
                        <div class="d-flex align-items-lg-stretch col-12 mb-4 {{ $i % 3 ? 'col-md-4' : 'col-md-8' }}">
                            <div class="card w-100 border-0 overlay-bg img-hover "
                                @php $bgimg =  asset('/storage/'.$item->poster); 
                                    if( !empty(json_decode($item->venues[0]->images, true)) )
                                        $bgimg = '/storage/'.json_decode($item->venues[0]->images, true)[0]; @endphp
                                style="background-image: url({{ $bgimg }}); background-size: cover;">

                                <span class="single-name"></span>
                                <div class="pt-4 ps-4 pb-18 z-1">
                                    <h3 class="text-white mb-0">{{ $item->venues[0]->title }}</h3>
                                    <p class="text-white">{{ $item->venues[0]->city }}</p>
                                </div>

                                <a class="stretched-link"
                                    href="{{ route('eventmie.venues.show', [$item->venues[0]->slug]) }}"></a>
                            </div>
                        </div>
                    @endforeach
                </div>
