...
<body {!! is_rtl() ? 'dir="rtl"' : '' !!}>
    ...
    <div style="max-width: 680px;margin: 0 auto;">
            ...
            <tbody>
                <tr>
                    <td class="border-0 pl-0 name-info">
                        ...
                        <p>@lang('eventmie-pro::em.order_date')
                            <strong>{{ \Carbon\Carbon::parse($bookings[0]['created_at'])->translatedFormat(format_carbon_date(true)) }}</strong>
                        </p>
                    </td>
                </tr>
            </tbody>
        ...
        <table class="table table-items" style="border-bottom: 1px solid #dee2e6;">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-right pr-0 total-amount">{{ $bookings->sum('net_price') }}
                        {{ $bookings[0]['currency'] }}</td>
                </tr>
        ...
        @if ($organizer->seller_signature)
            <table
                style="padding: 0;margin: 0;margin-top: 10px !important;margin-bottom: 10px !important;width: 100%;border-bottom: 1px solid #dee2e6;">
                <tr>
                    <td class="title-bar">
                        <table style="padding: 0;margin: 0;width: 100%">
                            <tr>
                                <td style="float: right;">
                                    @php
                                        $seller_signaturePath = public_path('/storage/' . $organizer->seller_signature);
                                    @endphp
                                    @if (file_exists($seller_signaturePath))
                                        <img style="width: 64px;text-align: center;margin-bottom: 5px !important;"
                                            src="{{ 'data:image/png;base64,' . base64_encode(file_get_contents($seller_signaturePath)) }}">
                                    @else
                                        <p>Event image not found</p>
                                    @endif

                                    <p class="m-heading" style="font-size: 12px;font-weight: 600;">@lang('eventmie-pro::em.seller_signature')
                                    </p>
                                </td>
                            </tr>
                        </table>
